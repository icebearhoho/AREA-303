...
### `shopee_id_products_clean.parquet`  —  Shopee Indonesia cosmetics/skincare product-day snapshots (IDR), 10 shops x 3 days, real daily-tracked demand signal.
...
| `monthly_sold_value` | float64 | Units sold in the last 30 days — the demand signal for this idea. | e.g. 1000.0 |
...
### `shopee_id_category_list_clean.parquet`  —  Per-shop category taxonomy with daily item counts (secondary/category-level trend signal).
...
| `total` | int64 | Number of items in this category (tracked daily — a rough proxy for category growth/shrink). | e.g. 5 |
...
shopee_id_category_list = pd.read_parquet(r"shopee_id_category_list_clean.parquet")   # or pd.read_csv(r"shopee_id_category_list_clean.csv")
```

## Watch out for
- fashion_trend is small (660) and looks synthetic — prototype only
- shopee_id is only 3 days of history (2026-07-01/02/03) — not enough for real seasonality, but
  monthly_sold_value/history_sold_value are genuine transaction-derived demand counts, unlike fashion_trend's
  synthetic sales_count
- shopee_id is cosmetics-only, 10 shops — a supplementary real-demand signal, not a primary training set
...
4. Use shopee_id.monthly_sold_value as a sanity-check / secondary demand target when validating the model
   against real (if short-window) transaction data
